...
- [x] fresh監査×3 完了: **3ツールとも全基準合格**。
  A: 監査官が自作DXF（mm・+120000/+45000オフセット・37°回転）で独立検証、
  誤差0.000000m・回転保持。B: 3600件の t を生データから全再計算し完全一致、
  ks_p は scipy と 1e-9 一致、別形式ExcelをYAMLだけで吸収。C: オフライン
  コンテキストで外部リクエスト0件を実証、rAF自前ラップで 59.87fps、
  全KPIを生ログから独立再計算（差は1,301km中2mm=Float32）。
- [x] 監査の軽微指摘3件を修正（hms繰り上がり・入れ子分布注記・mapping型検証）。
...
- 初回起動の3エージェントはセッション上限（resets 3pm UTC）で途中死。作業ツリーの
  部分編集は構文チェック済みで無事 → 巻き戻さず SendMessage で**同エージェントを
  続きから再開**（transcript に文脈が残る）。以後も上限で切れたら同じ手順で再開する。

## 完了

- [x] U1/U2/U3 完了（セッション上限の中断→再開を含む）。実バグ12件修正・
  コミット push 済（UX総合見直し）。
- [x] fresh UX踏破監査: 8タスク中 合格2・条件付き6 — 重要残件5件
  （③採用の嘘ボタン/多重実行/取込カード矛盾/未実行DLの沈黙404/誤読数値）。
- [x] 残件5件を修正（adopt.js=全採用ボタン共通の誠実性ガード新設）、
  headless before/after 検証、コミット push 済。
- 軽微な積み残し（監査の補足より）: 採点表ストリップの値切れ・寸法変更後の
  ステータスバー旧値・PPTXスライド番号重複・②ピルの早すぎる✓ 等 — 次回パスへ。
