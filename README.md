...
確実な自社マスタを軸に、楽天/Yahoo をベストエフォートで補完できるようにしています。
...
| `--providers LIST` | 取得元をカンマ区切りで指定（既定 `local`） |
...
```
Excel(JAN列) → LookupPipeline ──▶ Cache(SQLite) ──▶ 各Provider(順にフォールバック)
