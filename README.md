...
専門知識がなくても、チャットだけで一周できます。画面は ①取込 → ②分析 → ③設計 →
④検証 → ⑤提案 の5フェーズをガイド付きで進む動線で、いま何をすべきかが常に示されます。
設計を詰めたいときは ③設計 フェーズでレイアウト・設備・フローを直接編集します（下記）。
Cody チャットと「知見」は全フェーズから横断で使えます。
...
### Logi3D 由来の機能（レイアウト＆プレゼン強化）

メガソフト「物流倉庫3D」を参考に、提案・見栄え面を強化：

- **躯体設計**：設計タブの「躯体」で外壁（ポリライン）とドック扉/通用口/シャッターを作図。
- **マテハン機器パーツ**：フォークリフト・ロボットアーム・自動倉庫(ASRS)・ホイストクレーン・
  コンベアを配置し、2D/3Dに**3Dモデルで表示**。
- **3D表現プリセット**：ナチュラル／夕／夜／モノ（Logi3Dのアートフィルター相当）。
- **エクスポート**：KPI CSV・**動線一覧CSV**（移動距離/時間/回数、Excel可）・印刷用**提案書**。
- **CAD取込**：DXF図面を取り込み、外形/壁/ゾーンを自動生成（mm/m単位を自動判定、寛容）。
- **編集可能な提案書**：**PPTX**（PowerPoint）と**PDF**を生成（KPI表＋レイアウト図＋前提、日本語フォント）。
- **動線ツール**：任意経路を引いて距離・所要時間を一覧化（Logi3Dの動線一覧相当）。
- **フォークリフト個体走行**：入荷格納の動線を出荷量に連動して再現、2D/3Dでアニメ表示。
- **3D品質**：影・PBRマテリアル・床テクスチャ・マテハン機器の3Dモデルで提案グレードに。

> 依存追加：CAD/提案書出力には `pip install -e ".[web,docs]"`（ezdxf / python-pptx / reportlab）。
