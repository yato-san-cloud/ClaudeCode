...
- **OCTA チャットで操作** — マスコット「OCTA」に日本語で頼むだけで、作成・実行・分析・比較・概算ができる（LLM 差し替え可能な seam）。
- **テンプレート** — 全項目が仮値で埋まった `model.json`。取り込み前から必ず動く。
- **実データ取込（堅牢）** — 出荷/入荷の実 WMS 帳票（CSV/Excel、`.xls` 含む。タイトル行・合計行・半角カナを自動処理）、商品マスタ、MapMaker **ネイティブ `.rmpm`**（Java直列化を直接解釈）/ `.rmpm.json` / 地図CSV、DXF 図面（DWG誤保存を検知）、実測距離行列を寛容に取り込み。①取込はカテゴリ別の1画面ハブ。
- **解析→DESの二段構え（SLC流ジャーニー）** — ①基礎物量（物量分析→生産性ライブラリで爆速解析）→②**生産性試算**（MapMaker距離×動作時間でオーダー/マルチ/トータルを DES なしで即比較）→③**DES**で裏取り（作業方法比較で解析推奨と突合せ）。
- **採点表レール** — 設計の従属変数（判定・人員・原価・生産性・坪数・エリア連鎖）を編集のたび解析的に再計算し右ドックに常設。折り畳み/分割でき、現行 vs 代替案をシナリオで横並び比較。
- **設計エディタ（CAD代替）** — ライブラリ＆ホットバー型の配置（棚9種/ゾーン/マテハン設備/躯体をクリック装備 or ドラッグ）。グリッド・スナップ・寸法表示・工程↔エリアの整合チェック、通路網の自動経路生成（A→B計測・工程フロー動線）。
- **DES シミュレーション** — SimPy による離散事象シミュ。作業員・AGV・フォークリフトは個体エージェント。
- **2D / 3D リプレイ** — 同じ replay データを 2D アニメ（GIF も可）と three.js の 3D で描画（人/フォークリフト/AGV が別形状）。
- **原価試算（6費目）** — LOGISTEED 試算フローの作業費/保管費/輸配送費/システム費/運営費を解析的に積み上げ（実行不要・爆速）。3層生産性（実測採用＞物流形態ベンチマーク＞既定）。
