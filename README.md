...
4. 手順ごとに「手順／⚠注意／☑点検」を選べる（点検はチェックリストになる）。似た手順は⧉で複製
5. 「⬇ HTML書き出し」で完成品を1ファイル保存 → Teams/Kintone/メールで共有。書き出し後に編集すると一覧に「✎ 未提出の変更」が表示され、提出忘れを防げる
6. 本数が増えたら一覧上部の検索でタイトル・設備名から絞り込み
