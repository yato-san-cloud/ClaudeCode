...
17. **ライン運用の3機構は「意思」であって幾何ではない**（`Conveyor.stop_gate`/`load_kind`、`Process.container_pool`、`Process.divert_policy`）。1本のベルトの上を2種類の荷が同時に流れ、容器は有限で循環し、引き込みは自動ではなく人が引く——どれも**図面には描けない**（描けるのは線と箱だけ）。**全部 opt-in・既定オフで、同梱テンプレートは全部オフ**（HEAD の worktree に対してイベントログ/キーフレーム/replay/判定文までダイジェスト一致を確認済み。KPI が additive キーを 10 個増やすだけ）。
    ...
    - ⚠️ **未達**: `analytic.py` はこの3つを**まだ鏡写しにしていない**。既定オフなのでカタログの一致ピン（不変条件5）は無傷だが、**どれかを ON にしたモデルは解析だけ甘い数字を出す**。ON にする者が次に埋めるべき穴。3つとも閉形式は導出・DES突合済（scratchpad; 選択停止 |Δ梱包稼働率| 平均0.010・最大0.028、容器 滞留0.4%中央値、pull 引込率 rms 0.011）。
    - 🔴 **より広い穴が実測で出ている**: `_steady_block` は **既定の `auto` でも DESより甘い**。引き込みへ `lam/本数` を均等に配って各々の `_mmck_full` を平均するが、貪欲ディバートは**空きのある最初の分岐**を取るので実際は前詰め（ρ_bank=1.0 で実測 236/229/189）。ρ_bank 0.85〜1.05 の帯で 63条件中22件が甘い側、最悪 −0.577。同梱 `line_inspection` を2倍需要で回すと実測 0.467 に対し 0.065（**7倍甘い**）。docstring の「意図的な上界」は**容量以下でしか成り立たない**（飽和域では bank 全体が詰まるので、プールした真値の方が分割推定より遥かに悪い＝上界が下界に反転する）。プールした `M/M/Σc/ΣK` に置き換えても改善しない（実測: 31/63 が甘い側）ので、**逐次オーバーフローの待ち行列縦続として導出し直す**必要がある。既定経路なので影響範囲は3機構より広い。
18. **MapMaker の意味は名前にしか無い**（`rmpm.classify_name` / `_NAME_RULES`）。形式には z 座標が無く、種別も3つ（棚/壁/作業台）しか無い。だから2段駆動コンベアの上下段は**同一XYの2オブジェクト**、停止線と仕切りは**WallObject だが障壁ではない**、人の立ち位置は**500mm角の作業台**として描かれる——形式に無い情報は、**作図者が名前に書く**しか置き場が無い。判定は**1つの表**に集約し、**名前が先・`type` が fallback**（規約語ゼロの図面はバイト同一）。名前は「**何であるか（どこに・どうする）**」の形なので、**注記（括弧・｜ の後ろ）より先に頭を見る**——注記は平気で他の設備を名指しし（`検品者立ち位置マーカー(西・仕切り付近)` は人であって区画線ではない、`投入口(検品済オリコン→下段本線)` はシュートであって本線ではない）、全文一致だと**人が1人消える**。作図の都合で半分ずつ描かれたベルト（`北半`/`南半`）は結合するが、**両方の名前が相方を名指しする**（`北半…｜南半と一体`）ので**全ての半語を消してから**キーを作る（1つだけ消すと永久に揃わない）。**新しい語を足すなら、この表と `tests/test_rmpm_naming.py` の両方に足す**。
