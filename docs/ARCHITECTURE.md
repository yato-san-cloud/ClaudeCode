...
17. **ライン運用の機構は「意思」であって幾何ではない**（`Conveyor.stop_gate`/`load_kind`、`Process.container_pool`、`Process.divert_policy`、`Process.release_schedule`、`Process.bench_staging`）。1本のベルトの上を2種類の荷が同時に流れ、容器は有限で循環し、引き込みは自動ではなく人が引き、末端のストッパーは選り分けずに全部止め、完成品は台の脇に溜めて周期的にまとめて流す——どれも**図面には描けない**（描けるのは線と箱だけ）。**全部 opt-in・既定オフで、同梱テンプレートは全部オフ**（HEAD の worktree に対してイベントログ/キーフレーム/replay/判定文までダイジェスト一致を確認済み。KPI が additive キーを増やすだけ）。
    ...
    - ⚠️ **物理ストッパー / 時間分離リリース / 完成品staging に解析側の鏡は無い。`analytic` は名乗って降りる**（`_unmirrored_line_mechanics` → `conveyor.line_mechanics_mirrored: false` ＋ `unmirrored: [...]`）。降りる先は歴史的な連鎖の答え（`_conveyor_estimate`）で、**`linemech.gate` も `linemech.pull` も使わない**——どちらの導出もこの形の線では成り立たないから: pull は「引かれなかった荷は失われる」損失系だが**ストッパーはそれを待ち行列に戻す**（後で引ける・開放で流れる）し、停止線の式は**ゲートに立つ手をサーバに置く**が物理ストッパーに番人は居ない。**黙って別の機構の式を当てるのが一番悪い**ので、宣言して DES に渡す。宣言キーは**降りたときだけ**出る（何も書いていないモデルには出ない＝同梱カタログはバイト同一）。
      ✅ **ただし「降りた」と「降りた先が甘くない」は別の主張で、後者は測るまで成り立っていなかった**。完成品staging が張られた図面で、降りた先（`min(ベルト段, μ_pack)`）は**実測スループットを平均+472%・最悪+1614%上回っていた**（周期1200s・置き場1個/台の合成ラインで「60件/h」と言い、走らせると 8.5件/h）。3機構のうち**閉形式が実際に在ったのは1点だけ**で、それが `analytic._release_drain`＝**完成品staging の排出天井**。理由は待ち行列ではなく**保存則**である: 完成品は置き場を通ってしか出荷に出られず、置き場は**リリースの窓の中でしか空かない**（`processes._load_staging` は `release_agent` の窓の中でしか `get` しない）。しかも drain は**全部の置き場が空になった瞬間に break する**ので、1つの窓で出て行くのは「置き場の総数 `room`」＋「満杯で put をブロックされている梱包者 `hands`（梱包し終えて手に持ったまま止まっている1個）」＝ **X ≤ (room + hands) / 周期**。窓長には依らない（窓を伸ばしても drain は空になった時点で止まる）ので、**棄却された `capacity × (1 − 窓の割合)` とは別物**。⚠️ **`per_hr` は上界ではなく較正値**（break の議論は drain がシミュ時間を消費しないときに厳密で、本線に空きが無くて待つ窓では甘くなる）。証明できる上界 `room + hands + 窓長×μ_pack`（`board_time_s` があれば直列性で更に締まる）は `bound_per_hr` として**並べて開示**し、能力に効くのは `per_hr` の方＝`min()` が誠実さの本体。もう1本、**証明できる天井**が `stack_per_hr`＝`stackers × stack_rate_per_hr`（出て行った完成品は全部 `processes._stack_out` を通る。未指定なら制約ではないので `None`）で、これは `per_hr` にも `min` で入る。実測（288構成＝周期3×置き場4×引き込み3×台数2×窓割合2×board2、需要飽和3000件/h、8h、2seed）: 平均 **+472%→+19.8%**、最悪 **+1614%→+111.8%**、天井が縛る240構成では 平均 +9.7%・最悪 +68.0%・最小 −15.0%（辛い側＝能力の天井としては安全側）が14構成。**staging を張っていない図面（ストッパー単独/リリースのみ）では天井は計算されず数字は不動**——そこは実測64構成で歴史的な答えが 1.01〜1.81倍、甘い側に外れた構成は0だった。`binding: "staging"` が縛っていることを名乗る。回帰は `tests/test_analytic_release_drain.py`。
      ✅ 付随して**述語の写しのドリフトを4つ閉じた**（不変条件11）: `_gate_reading` に1本化して「`{"mode":"everything"}` は build ではゲートではないのに在ると読む」「**選択停止ゲート＋リリース**は build では列が育ち周期で開くのに `stopper` を名乗らない」「停止線が1本も無いのに `bench_staging` を名乗る（engine は張らない）」の3つ、そして `_wired_belts` を `_belt_stages` から切り出して「**どの `flow_edge` も通さないベルトの停止線**」を閉じた——`build` は `flowgraph.conveyor_ids_in_use` で絞ってから配線するのに述語は `resources.conveyors` を丸ごと読んでいたので、**存在しない機構を理由に降りていた**（`_belt_access` の「片方にしか見えないベルト」と同じ形。不変条件13）。parity は `test_the_predicates_read_the_same_drawing_the_engine_wires` と `test_a_gate_on_a_belt_the_flow_does_not_route_through_is_not_a_gate` が固定する。
      ⚠️ **降りると `divert_policy` に関わらず `_overflow_cascade` が当たる**（`auto` の水詰めが pull のラインに当たる）。「pull は `linemech.pull` が先に取るので届かない」という `_overflow_cascade` の前提は、**降りる機構が出来た時点で偽になっていた**。方式で分岐せず測った: 144構成（方式2×需要3×引き込み3×台数2×staging有無×周期2、8h、2seed、全構成ストッパー在り）で `block_ratio_est` − 実測は pull **平均 +0.436・範囲 −0.159〜+0.689**、auto 平均 +0.289・範囲 −0.162〜+0.655＝**pull へ当てても辛い側**。甘い側は各方式 1/72 でしかも**同じ構成が auto でも同じだけ甘い**ので、原因は方式ではなく「ストッパーの列が本線のスロットを握る」こと（縦続は列を持っていない）＝鏡の無い機構そのもの。だから分岐は足さず、**何が起きているかを書く**。
...
- `engine/`：`processes.py` / `graph.py`（壁考慮グリッド＋Dijkstra＝**timing権威**）/ `navnet.py`（MapMaker風 Delaunay waypoint網＝**viz層**、facing開放面ピック点）/ `build.py` / `run.py` / `routing.py` / `pickroute.py`（S字/折り返し/最大ギャップの訪問順序＝純関数; `process.routing_policy` で切替、既定 nearest は不変。`routecompare.py` が4方式＋2-optの距離比較表） / `scenarios.py`（dotted-path what-if ＋ `whatif(base, edits)`=diff→ヘッドレス実行→サマリ）。
  ...
  **解析側の扱いは2つの機構で違う**（`tests/test_analytic_aisle_interference.py`）: **通路干渉は上界として映す**（`analytic._aisle_congestion`。セル1つ＝容量1のM/M/1、格子ピッチは約分で消えるので**歩行「時間」の倍率**であって距離は1mも動かさない。ρ は Little から `λ·travel/v ÷ 2·通路数·ℓ/g`、外周一周を下限に置く。同速の追従は自己消滅する—一度待てば以後は1セル後ろを付いて行くだけ—ので実測 9.1〜23.0倍 甘くない側に外れる。秒では緩いが真の効果が歩行時間の0.03〜0.17%なので**稼働率としては丁度**）。**非既定ルーティングは測った上で映さないと決めた**: DES自身の感度が9テンプレ中7で歩行の1.5%以下・稼働率で0.013以下（一致ピン0.08の6分の1）、エンジン自身の `route_order` から組んだ閉形式は**オラクルを悪化させ甘い側へ倒した**（平均 6.8%→10.0%、最大 15.5%→31.0%; `_batch_per_trip` のバッチ誤差を非線形に増幅し、同梱テンプレの出荷口が全部ラック帯の中ほどを向いているため）。代わりに `routing_policy`/`routing_policy_mirrored` を**名乗る**。⚠️ **`apparel` は s_shape、`food_chilled` は return を既に使っている** — 「既定nearestだから無害」は事実として誤り。**AGV同士の干渉（`process.agv_interference`）も測った上で映さないと決めた**（`analytic._agv_interference`／`tests/test_analytic_agv_interference.py`）。歩行側と同じ形（セル1つ＝M/M/1）を実装して測ったら**逆向きに外れた**: 上界/実測の待ち＝**0.055〜0.17**（合成の一本廊下 AGV2〜12台の12構成 ＋ `ecommerce_xl` AGV2〜32台×需要3種の15構成）＝実測の 1/6〜1/18 しか言えず、**甘い側に外れる上界は上界ではない**（窓の割合の件と同じ失敗の形）。歩行側の導出の前提が2つとも崩れるからで、①歩行の「同速の追従は自己消滅する」が効かない——AGVのロック(`processes._agv_seg_key`)は**コーナー間のレグ丸ごと**を保持するので後続はレグ1本分待たされ**直列化する**、②鍵はレグ中点の3m量子化なので通路1本の交通が数個の鍵に集中し、床の平均ρ（実測 0.002〜0.03）はホットな鍵のρ（ほぼ1）と2桁違う——**どれが熱いかは経路探索でしか分からず、解析は50ms予算で経路探索をしない**。逆の極（全レグが共有廊下＝完全直列化、`1/trip_s`）は上界ではあるが `ecommerce_xl` で 19.9件/h と読む一方 実測 124〜379件/h＝**6〜19倍辛い**ので答えを破壊する。一様分散は10倍甘く完全直列化は6〜19倍辛い、その間は経路の形で決まる ⇒ `routing_policy` と同じく **`agv_interference` として名乗る**（フラグONのときだけ出るキー＝カタログはJSONまでバイト同一）。開示する実測: 通路待ちはAGV busy の **1.5〜35.7%**、`agv_utilization` の動きは **−0.025〜+0.044**（一致ピン0.08の内側・符号は両方向）、スループットは **−2〜−34%**（台数が増えるほど効く）。⚠️ **能力側は映していない**ので大きな船団では DES の方が出荷が少ない。稼働率は自由走行の読みが実測を15/15構成で下回らなかった（最小マージン +0.005）が、**測っただけで証明ではない**。
