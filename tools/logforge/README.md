...
### 入れ子の分布族について（監査で確認された性質）

expon は gamma(a=1) の特殊形なので、真値が expon の系列でも KS p値だけで選ぶと
gamma(a≈1.0) が僅差で勝つことがある（分布としては等価 — 分位の相対誤差 ≤1.4% を実測）。
族の同定が目的なら `candidates` を見て AIC 等で判断すること。フィット値を
シミュレーションの入力に使う分には、どちらを選んでも実用上の差は出ない。
