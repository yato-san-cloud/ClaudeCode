...
- `src/analyses.py` — pandas 純粋関数 4 本: `volume_trends`, `abc_analysis`, `peak_analysis`, `inventory_turnover`。標準化された論理列名 (`date`, `timestamp`, `sku`, `qty`, `partner`, `location`) を入力に取る
- `src/sql_analyses.py` — 同じ 4 分析の SQL 版。`Catalog` を受け取り、集計後の小さな DataFrame のみを materialize
