...
extends AbstractRectangleObject {
    ...
    static {
        int size = 3;
        while (size <= 100) {
            FreeShelfObject.fontForSize[size - 3] = new Font("Arial", 0, size);
            ++size;
        }
    }

    public FreeShelfArea shelf() {
        return this.shelf;
    }

    public void setShelf(FreeShelfArea shelf) {
        this.shelf = shelf;
    ...
    public StringLabel[] labelCands() {
        if (this.labelCands == null) {
            this.labelCands = new StringLabel[]{new StringLabel(this.shelf.name)};
        }
    ...
    public static void paintShelfObjects(Collection<FreeShelfObject> shelfObjects, Graphics2D g, MapView mapView, boolean showLabels, boolean showStartEndShelf) {
        int screenWidth = mapView.getWidth();
        int screenHeight = mapView.getHeight();
        if (showLabels) {
            int fontSize = 100;
            for (FreeShelfObject obj : shelfObjects) {
        ...
        for (FreeShelfObject obj : shelfObjects) {
            if (!showStartEndShelf && obj.shelf.equals(FreeShelfArea.pickingStartEndShelf)) continue;
            obj.paint(g, mapView);
        }
    }
    ...
    public AbstractObject hardClone() {
        FreeShelfArea newShelf;
        FreeShelfObject newObj = new FreeShelfObject();
        newObj.shelfColorRGB = this.shelfColorRGB;
        newObj.shelf = newShelf = new FreeShelfArea(this.shelf.name);
